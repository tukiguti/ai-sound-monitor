...
//   /name set    … 表示名・読み上げ名(config/names.json)を変更する(再起動不要)
//   /name list   … 現在の名前マップを一覧表示する
// テキスト通知先は .data/discord-bot-state.json に保存し、サーバ再起動をまたいで覚えておく。
// 読み上げ設定の読み書きは lib/voice-settings.js に、名前マップ(config/names.json)の読み書きは lib/names-store.js に
...
const DISCORD_FILE = path.join(__dirname, '..', 'config', 'discord.json');
const STATE_FILE = path.join(__dirname, '..', '.data', 'discord-bot-state.json');   // テキスト通知先の永続化
...
// 設定を1項目読む: 環境変数が最優先、無ければ config/discord.json。
// config/discord.json.example のプレースホルダ(全角「（」で始まる説明文)はコピーしただけの未設定として扱う。
function readSetting(envValue, fileKey) {
  ...
  } catch { /* config/discord.json が無い/壊れていれば未設定として扱う */ }
  return '';
}

// このツールの唯一の資格情報。環境変数が最優先、無ければ config/discord.json の botToken。
...
// テキスト通知先チャンネルIDを .data/discord-bot-state.json から読む(無ければ空)。
...
function registerCommands(discordjs, guild) {
  ...
  const commands = [
    ...
    // 変更は config/names.json に保存され、次のイベントから反映される(再起動不要)。
    new SlashCommandBuilder()
      .setName('name')
      .setDescription('表示名・読み上げ名マップ(config/names.json)を変更する(再起動不要)')
      .addSubcommand((sub) =>
        sub
          .setName('set')
...
// 実際の読み書きは lib/names-store.js に任せる(config/names.json に保存され、次のイベントから反映される)。
async function handleName(interaction) {
  ...
  setName(base, label);   // config/names.json に保存し、次のイベントから反映される
...
// 実際の読み書きは lib/voice-settings.js に任せる(config/config.json ではなく .data/voice-override.json に保存される)。
async function handleVoice(interaction) {
  ...
  const voice = setVoiceConfig(partial);   // .data/voice-override.json に保存し、次の読み上げから反映される

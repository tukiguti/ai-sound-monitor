...
//   環境変数 VOICEVOX_SPEAKER(話者のみ・最優先) > .voice-override.json > config.json の既定値
//
// config.json は「コミット対象の既定値」なので、この機能からは絶対に書き込まない。
// /voice set で変更した値は、gitignore 済みの .voice-override.json にだけ書き出す
...
const OVERRIDE_FILE = path.join(__dirname, '.voice-override.json');   // /voice set の保存先(gitignore済み)

// --- 基本の既定値: config.json の voice を読む(既存の server.js と同じ挙動) ---
...
// --- 上書き設定: .voice-override.json を読む(あれば既定値へ被せる) ---
...
// 上書き設定を .voice-override.json へ保存する(投げっぱなし・失敗してもサーバは止めない)。
function saveOverride() {
  writeFile(OVERRIDE_FILE, JSON.stringify(override, null, 2) + '\n', () => {});
}

// 現在有効な { speaker, volume, speed } を返す。
// 優先順位: 環境変数 VOICEVOX_SPEAKER(話者のみ・最優先) > .voice-override.json > config.json の既定値。

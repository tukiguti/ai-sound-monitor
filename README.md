...
- **3分（180秒）未満の短いターンでは、読み上げ・Discord投稿を抑制する**（ブラウザのチャイムと盤面表示は毎回動く）。UserPromptSubmit がターン開始時刻を `/tmp/asm-turn-<sid>` に記録し、Stop で経過時間を測って180秒未満なら `quiet=1` を付けて送る仕組み。**閾値は hook 設定内の `180` を書き換えれば変更できる**（開始時刻ファイルが無い＝計測できない場合は従来どおり鳴る）。
...
読み上げ・Discord を「意味のある区切り」だけに絞るための仕組みで、`quiet=1` を受けてもサーバは盤面更新・SSE配信（チャイム）は従来どおり行う。
...
- 「一段落」は**応答ターンの終了**を表す（Claude Code にタスク完了のイベントは無いため）。3分未満の短いターンでは読み上げ・Discord投稿を抑制する（上記「静かな通知」）。チャイムと盤面は毎回動く

...
//   ⑤ 状態を .data/state.json に保存し、サーバ再起動後も盤面を復元する
//   ⑥ VOICEVOX で「〇〇が一段落しました」と読み上げる (ENGINE未起動なら自動スキップ)
//   ⑦ config/names.json でAI名を表示名に変換し、同名が複数あるときは番号(1,2,3…)で区別する
//   ⑧ Discord Bot へ一段落・承認待ち・エラーを投稿する (通知先は /notify here で指定・未指定なら何もしない)
//   ⑨ config/config.json で話者・音量・速さ・チャイム音量を設定できる(話者・音量・速さは Discord の /voice set でも再起動なしに変更可)
//
// 状態の特別扱い:
...
import { initBot, playInVoice, isBotConfigured, sendText } from './lib/discord-bot.js';
import { getVoiceConfig } from './lib/voice-settings.js';
import { getLabel, registerName } from './lib/names-store.js';

const PORT = process.env.PORT || 4123;
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PUBLIC_DIR = path.join(__dirname, 'public');

// 実行時状態(盤面・通知先・読み上げ上書き)の置き場。gitignore 済みで、丸ごと使い捨てできる。
// 無いディレクトリへの writeFile は失敗するため、どの書き込みよりも先に一度だけ作っておく。
...
// マップの読み書き(config/names.json)は lib/names-store.js に集約している(server.js ⇄ lib/discord-bot.js の
// 循環 import を避ける共通置き場。lib/voice-settings.js と同じ考え方)。
...
// --- ⑧ Discord通知は lib/discord-bot.js が担当(テキストは sendText / ボイスは playInVoice)。
...
async function speakOnce(text) {
  ...
  query.volumeScale = voice.volume;                           // 音量・速さは現在の設定から(lib/voice-settings.js)
...
function handleEvent(event) {
  ...
  registerName(base);              // 未登録なら config/names.json に自動追記(登録済みなら何もしない)
...
server.listen(PORT, '127.0.0.1', () => {   // localhost(ループバック)のみで待受(認証なし前提・同一LANからは叩けない)
  ...
  console.log(isBotConfigured()
    ...
    : '  Discord Bot : 未設定(config/discord.json か DISCORD_BOT_TOKEN で有効化・要 npm install)');
  console.log('==============================================');
